...
# Train MLP Model
def train_model(model: nn.Module, train_iter, test_iter, EPOCHS:int, BATCH_SIZE: int, device: str):
    print_every = 1
...
def test_model(model, test_iter, device:str):
    ...
    for idx in range(n_sample):
        ...
        plt.axis('off')
        plt.title(f'Predict: {y_pred[idx]}, Lable: {test_y[idx]}')
    plt.show()
...
if __name__ == "__main__":
    ...
    train_iter, test_iter = get_dataloader(config.BATCH_SIZE)
    print("Preparing dataset done!")

    network, criterion, optimizer = get_network(config.LEARNING_RATE, config.device)
    print_model_info(network)

    train_model(network, train_iter, test_iter, config.EPOCHS, config.BATCH_SIZE, config.device)
